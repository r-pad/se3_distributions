...
def main():
        ...
        for obj_id in tqdm(object_list):
            ...
            for v_id in tqdm(dataset.getVideoIds()):
                ...
                for j in tqdm(range(len(dataset))):
                    ...
                    dists_found = True
                    for d_idx, path in enumerate(dataset.getPaths(j)):
                        try:
                            ...
                            if(d_idx == 0):
                                quat = torch.tensor(dist_data['quat'])
                    ...
                    err[obj_id][v_id].append(symmetricAngularDistance(mode_quat, quat,
                                          sym_axis, sym_ang).item()*180/np.pi)
                
                    err_max_shift[obj_id][v_id].append(symmetricAngularDistance(v_shift.cpu(), quat,
                                                    sym_axis, sym_ang).item()*180/np.pi)
    ...
    np.savez('results/hist_filter_{}_rot_{}x{}.npz'.format(save_key, video_len, interval), 
             lik=lik,
             err=err,
