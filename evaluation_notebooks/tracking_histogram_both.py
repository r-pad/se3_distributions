...
def combineHistBingham(dists, trans, grid_vertices, sigmas, confidence = None):
    num_dists = len(dists)
    if(confidence is None):
        confidence = np.ones(num_dists)
    ...
    for d, mat, w in zip(dists, trans, confidence):
        ...
        bing_interp = BinghamInterpolation(grid_vertices_inv, torch.Tensor(d), sigma=30)
        v_interp = tetra_interp.smooth(grid_vertices_inv)
...
def main():
                ...
                for j in tqdm(range(len(dataset))):
                    ...
                    for path in dataset.getPaths(j):
                        try:
                            ...
                            dist = dist_data['dist_est'].flatten()
                            if('pcnn_quat' in dist_data.keys()):
                                quat_est = dist_data['pcnn_quat']
                            else:
                                quat_est = dist_data['df_single_quat']
                            dist += to_np(isobingham_likelihood(torch.tensor(quat_est).flatten().cuda(), 
                                                          torch.tensor(sigmas_single[obj_id]).unsqueeze(0).float().cuda(),
                                                          grid_vertices.cuda()).flatten())
                    ...
                    dist_joint = torch.tensor(combineHistTetra(dists, trans, grid_vertices))
    ...
    np.savez('results/hist_filter_{}_both_{}x{}.npz'.format(save_key, video_len, interval), 
             lik=lik,
             err=err,
