...
def evaluate(data_loader, 
    ...
    for n, (images, trans, _, models,_) in enumerate(data_loader):
        import IPython; IPython.embed()
        for j, img in enumerate(unprocessImages(images[0])): 
...
def main():
    ...
    t = time.time()
    import IPython; IPython.embed()
    evaluate(data_loader, args.results_prefix, num_samples=args.num_samples)
