...
from generic_pose.training.max_angle_utils import evaluateMaxedTrueAxisAngle
...
#        train_loop_errors = []
        ...
        for epoch_idx in range(1, num_epochs+1):
            for batch_idx, (images, trans, quats, models, model_files) in enumerate(self.train_loader):
                ...
                train_results = evaluateMaxedTrueAxisAngle(model, images[0], images[1], trans[0],
                                                  optimizer = self.optimizer, 
                                                  disp_metrics = True)

                if(self.cross_model_eval):
                    train_cross = evaluateMaxedTrueAxisAngle(model, images[1], images[2], trans[1],
...
#                        train_loop_errors = []                                        
                    ...
                    for valid_logger, valid_loader in zip(valid_logger_list, self.valid_loaders):
                        ...
                        valid_results = evaluateMaxedTrueAxisAngle(model, v_images[0], v_images[1], v_trans[0],
                                                          optimizer = None, disp_metrics = True)

                        if(self.cross_model_eval):
                            valid_cross = evaluateMaxedTrueAxisAngle(model, v_images[1], v_images[2], v_trans[1],
                                                            optimizer = None, disp_metrics = True)
                        if(self.loop_training):
