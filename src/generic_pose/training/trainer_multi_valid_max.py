...
from generic_pose.training.utils import to_np, evaluatePairMaxReg, evaluateLoopReg
...
#        train_loop_errors = []
        ...
        for epoch_idx in range(1, num_epochs+1):
            for batch_idx, (images, trans, quats, models, model_files) in enumerate(self.train_loader):
                ...
                train_results = evaluatePairMaxReg(model, images[0], images[1], trans[0],
                                                   max_angle = self.angular_curriculum_limits[curriculum_idx],
                                                   optimizer = self.optimizer, 
                                                   disp_metrics = True,
                                                   clip = clip)

                if(self.cross_model_eval):
                    train_cross = evaluatePairMaxReg(model, images[1], images[2], trans[1],
                                                     max_angle = self.angular_curriculum_limits[curriculum_idx],
...
#                        train_loop_errors = []                                        
                    ...
                    for valid_logger, valid_loader in zip(valid_logger_list, self.valid_loaders):
                        ...
                        valid_results = evaluatePairMaxReg(model, v_images[0], v_images[1], v_trans[0],
                                                           max_angle = self.angular_curriculum_limits[curriculum_idx],
                                                           optimizer = None, disp_metrics = True)

                        if(self.cross_model_eval):
                            valid_cross = evaluatePairMaxReg(model, v_images[1], v_images[2], v_trans[1],
                                                             max_angle = self.angular_curriculum_limits[curriculum_idx],
                                                             optimizer = None, disp_metrics = True)
                        if(self.loop_training):
...
def main():
    ...
    if(args.angular_curriculum_limits is not None):
        angular_curriculum_limits = np.array(args.angular_curriculum_limits, dtype=float)
    else:
        angular_curriculum_limits = [np.pi/4.0]
    ...
    trainer = MultiTrainer(train_data_folder = args.train_data_folder,
                                       ...
                                       angular_curriculum_limits = angular_curriculum_limits,
                                       )
