...
from generic_pose.losses.feature_grid_loss import multiObjectLoss
...
class FeatureComparisonTrainer(object):
    def __init__(self, 
        ...
        self.model_datasets = [FeatureDatasetDebug(dataset_root = dataset_root,
                                                   feature_root = feature_root, 
                                                   feature_key = feature_key,
                                                   mode = 'train_sym',
                                                   resample_on_error = False,
                                                   object_list = [obj]) for obj in range(1,3)]
        ...
        for epoch_idx in trange(1, num_epochs+1):
            for batch_idx, data in tqdm(enumerate(self.train_loader), total = len(self.train_loader)):
                ...
                for idx in to_np(obj).flat:
                    grid_vertices.append(self.grid_vertices)
...
def main():
    ...
    model = SingleNet(args.feature_size, 12*3885)

#    if(args.dropout):
#        model = SigmoidCompareNet(args.feature_size, 21*3885)
#    else:
#        model = DetSigmoidCompareNet(args.feature_size, 21*3885)
