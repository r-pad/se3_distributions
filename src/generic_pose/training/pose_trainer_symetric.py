...
class PoseTrainerSymetric(object):
    def __init__(self, 
        ...
        self.valid_loader = DataLoader(DataSet(data_folders=valid_data_folders,
                                       ...
                                       batch_size=batch_size, 
                                       shuffle=True)
        self.valid_loader.dataset.loop_truth = [1,1]
        ...
        for epoch_idx in range(1, num_epochs+1):
            for batch_idx, (images, trans, quats, models, model_files) in enumerate(self.train_loader):
                ...
                if log_data:
                    ...
                    valid_results01 = evaluatePairReg(model, v_images[0], v_images[1], v_trans[0],
                                                      optimizer = None, disp_metrics = True)
                    valid_results10 = evaluatePairReg(model, v_images[1], v_images[0], v_trans[1],
                                                      optimizer = None, disp_metrics = True)
                    ...
                    if('loss_quat' in valid_results01 and valid_results01['loss_quat'] < min_loss_quat):
                        min_loss_quat = min(min_loss_quat, valid_results01.setdefault('loss_quat', -float('inf')))
...
def main():
    ...
    num_bins = (50,50,25)
    trainer = PoseTrainerSymetric(train_data_folders = train_data_folders,
