...
from quat_math import euler_matrix, quaternion_matrix 
...
class FinetuneYCBTrainer(object):
    def __init__(self, 
        ...
        #    renders = self.renderPoses(self.base_vertices, camera_dist = 0.33)
        #    self.base_renders = preprocessImages(render,
        ...
        if(os.path.exists(os.path.join(base_render_folder, 'renders.pkl'))):
            with open(os.path.join(base_render_folder, 'renders.pkl'), 'rb') as f:
                base_renders = pickle.load(f)
            self.base_vertices = torch.load(os.path.join(base_render_folder, 'vertices.pt'))
        else:
            self.base_vertices = np.unique(self.grid.vertices, axis = 0)
            ...
            for j, q in enumerate(self.base_vertices):
                ...
                base_renders.append(rendered_img)

            import pathlib
            pathlib.Path(base_render_folder).mkdir(parents=True, exist_ok=True)
            with open(os.path.join(base_render_folder, 'renders.pkl'), 'wb') as f:
                pickle.dump(base_renders, f)
            torch.save(self.base_vertices, os.path.join(base_render_folder, 'vertices.pt'))

        self.base_renders = preprocessImages(base_renders, img_size = self.img_size,
...
def main():
    ...
    parser.add_argument('--input_mask', dest='remove_mask', action='store_false')    
    ...
    if(not args.remove_mask and model.feature_network[0].in_channels == 3):
