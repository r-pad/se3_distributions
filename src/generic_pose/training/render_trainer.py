...
class RenderTrainer(object):
    def __init__(self, 
                 ...
                 renders_folder = None,
                 camera_dist = 0.2,
                 tensor_camera_dist = 0.4,
                 render_offset = None,
                 valid_render_offset = None,
                 brightness_jitter = 0,
                 contrast_jitter = 0,
                 saturation_jitter = 0,
                 hue_jitter = 0,
        ...
        valid_renders_folder = os.path.join(renders_folder, 'valid')
        self.valid_dataset = TensorDataset(valid_renders_folder,
                                              object_model,
                                              camera_dist = tensor_camera_dist,
                                              img_size=img_size,
                                              offset_quat = valid_render_offset,
                                              base_level = 0,
        ...
        train_renders_folder = os.path.join(renders_folder, 'train')
        self.train_dataset = TensorDataset(train_renders_folder,
                                              object_model,
                                              camera_dist = tensor_camera_dist,
                                              img_size=img_size,
        ...
        print("Validation Dataset Size:", len(self.valid_dataset)) 
        base_render_folder = os.path.join(renders_folder,
                                          'base_renders',
                                          '{}'.format(base_level))
        if(os.path.exists(os.path.join(base_render_folder, 'renders.pt'))):
            ...
            self.renderer.loadModel(object_model,
                                    emit = 0.5)
            ...
            for epoch_idx in range(1, num_epochs+1):
                for batch_idx, (query_imgs, query_quats, _1, _2) in enumerate(self.train_loader):
                    ...
                    if log_data:
                        ...
                        #########################################
                        valid_results = {}
                        for query_imgs, query_quats, _1, _2 in self.valid_loader:
                            del _1,  _2
                            torch.cuda.empty_cache()
                            res = evaluateRenderedDistance(model, #self.grid, self.renderer,
                                                                 query_imgs, query_quats,
                                                                 self.base_renders, self.base_vertices,
                                                                 loss_type = loss_type,
                                                                 falloff_angle = self.falloff_angle,
                            ...
                            for k,v in res.items():
                                if('vec' not in k):
                                    valid_results.setdefault(k, []).append(v)
                                else:
                                    valid_results.setdefault(k, []).extend(v)
                            torch.cuda.empty_cache()
                        valid_info = {}
                        for k,v in valid_results.items():
                            if('vec' not in k):
                                valid_info[k] = np.mean(v)
                            else:
                                #import IPython; IPython.embed()
                                #print(k, np.stack(v).shape)
                                #print(np.stack(v))
                                #print(np.max(np.array(v).flatten()))
                                #print(np.min(np.array(v).flatten()))
                                valid_logger.histo_summary(k, np.hstack(v), cumulative_batch_idx+1)
                        ...
                        if('loss' in valid_results and np.mean(valid_results['loss']) < min_loss):
                            min_loss = np.mean(valid_results['loss'])
...
def main():
    ...
    parser.add_argument('--object_model')
    parser.add_argument('--renders_folder', type=str, default=None)
    parser.add_argument('--camera_dist', type=float, default=0.2)
    ...
    parser.add_argument('--log_every_nth', type=int, default=500)
    ...
    if(args.random_render_offset):
        ...
        valid_render_offset = random_quaternion()
    else:
        render_offset = None
        valid_render_offset = None
    ...
    trainer = RenderTrainer(object_model = args.object_model,
                                      renders_folder = args.renders_folder,
                                      camera_dist = args.camera_dist,
                                      render_offset = render_offset,
                                      valid_render_offset = valid_render_offset,
