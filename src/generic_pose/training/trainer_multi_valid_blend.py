...
from generic_pose.training.utils import to_np, evaluatePairBlendReg, evaluateLoopReg
...
class MultiTrainer(object):
    ...
    def train(self, model, results_dir,
        ...
        if(self.angular_curriculum_limits is not None):
            train_logger.scalar_summary('max_orientation_offset', self.angular_curriculum_limits[curriculum_idx], cumulative_batch_idx+1)
...
#        train_loop_errors = []
        ...
        for epoch_idx in range(1, num_epochs+1):
            for batch_idx, (images, trans, quats, models, model_files) in enumerate(self.train_loader):
                ...
                train_results = evaluatePairBlendReg(model, images[0], images[1], trans[0],
                                                optimizer = self.optimizer, 
                                                disp_metrics = True,
                                                clip = clip)

                if(self.cross_model_eval):
                    train_cross = evaluatePairBlendReg(model, images[1], images[2], trans[1],
...
#                        train_loop_errors = []                                        
                    ...
                    for valid_logger, valid_loader in zip(valid_logger_list, self.valid_loaders):
                        ...
                        valid_results = evaluatePairBlendReg(model, v_images[0], v_images[1], v_trans[0],
                                                        optimizer = None, disp_metrics = True)

                        if(self.cross_model_eval):
                            valid_cross = evaluatePairBlendReg(model, v_images[1], v_images[2], v_trans[1],
                                                          optimizer = None, disp_metrics = True)
                        if(self.loop_training):
...
#                                              epoch_digits, batch_digits))
                    ...
                    if(self.angular_curriculum_limits is not None and 
                       ...
                       valid_results['mean_err'] < self.curriculum_threshold):
                        curriculum_idx += 1
