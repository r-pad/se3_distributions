...
from data_preprocessing import label2DenseWeights, resizeAndPad, uniformRandomQuaternion, transparentOverlay, quat2Uniform, randomQuatNear

class PoseRendererDenseDataSet(Dataset):
    def __init__(self, model_filenames, img_size, 
        ...
        super(PoseRendererDenseDataSet, self).__init__()
    ...
    def __getitem__(self, index):
        if(self.prerendered):
            ...
            offset_u = npzfile['offset_u']
            u_bins = npzfile['u_bins']
            
        else:
            origin_img, query_img, offset_quat, offset_u, u_bins = self.generateData(index)
        ...
        offset_u = torch.from_numpy(offset_u)
        u_bins = torch.from_numpy(u_bins)
                                      
        return origin_img, query_img, offset_quat, offset_u, u_bins
    ...
    def generateRotations(self):
        ...
        offset_u = label2DenseWeights(u_bins, self.class_bins)
