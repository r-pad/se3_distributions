...
def plotGroundTruthRank(img_prefix, gt_ranks, title_name = None,
    ...
    counts, edges = np.histogram(gt_ranks, bins = histogram_bins, range=(0,num_verts))
...
def translateFilename(filename):
    model, finetune_type = filename.split('/')[-5:-3]
    #import IPython; IPython.embed()
    if(finetune_type[-7:] == 'uni_0_5'):
        title_name = '50% Uniform '
    elif(finetune_type[-7:] == 'uni_0_5'):
        title_name = 'Hard Examples '
    elif(finetune_type[-8:] == 'temp_1_0'):
        title_name = 'Temperature 1.0 '
    else:
        title_name = finetune_type
    
    batch_size = finetune_type.split('_')[4]
    title_name += 'Batch Size {}:'.format(batch_size)
    ...
    return title_name, finetune_type, model
...
results_folder = '/home/bokorn/results/linemod_finetune/iron'
...
for filename in sorted(result_files):
    ...
    continue
    '''
    title_name, finetune_type, model = translateFilename(filename)
    if(plot_results):
        plotResults(img_prefix, data, title_name)
    #data_type = filename.split('/')[-3]

    if(False and last_data_type is not None and data_type != last_data_type):
