...
def main(weight_file,
    ...
    for obj in [13]:
        ...
        data_loader.dataset.setSequence('{:02d}'.format(obj))
        ...
        model_results_dir = os.path.join(results_dir, '{:02}'.format(obj))
...
if __name__=='__main__':
    #args = getArgs()
    import glob
    weight_paths = glob.glob('/home/bokorn/results/linemod_finetune/iron/**/best_quat.pth', recursive=True)
    for weight_file in weight_paths:
        results_dir = '/'.join(weight_file.split('/')[:-2])
        print(results_dir)
        main(weight_file = weight_file,
